...
uint64_t *receive(RH_RF95 rf96, uint8_t packet_len)
{
    if (rf96.available())
    {
        ...
        static uint64_t buf[MAX_PACKET_LEN];
    ...
    static uint64_t fail[1] = {1};
    return fail;
}

state get_state_from_bits(uint64_t packet)
{
    uint64_t extracted_state = Bitpack_getu(packet, 4, 60);
    return static_cast<state>(extracted_state);
}

// below is borrowed from direwolf
