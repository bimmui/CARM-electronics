...
def update_graph_live(n, hoverData):
    ...
    if hoverData:
        ...
        closest_index = min(range(len(data['Time'])), key=lambda i: abs(data['Time'][i] - x_value))
        altitude_value = data['Altitude'][closest_index]
        velocity_value = data['Velocity'][closest_index]

        altitude_hoverlabel = go.layout.Annotation(x=x_value, y=altitude_value,
                                                  text=f'Time: {x_value:.2f}<br>Altitude: {altitude_value:.2f}',
                                                  showarrow=True, arrowhead=4, arrowsize=2, arrowwidth=2,
                                                  arrowcolor='#636363', ax=0, ay=-40, bordercolor='#c7c7c7', borderwidth=2,
                                                  borderpad=4, bgcolor='rgba(255,255,255,0.8)', opacity=0.8)

        velocity_hoverlabel = go.layout.Annotation(x=x_value, y=velocity_value,
