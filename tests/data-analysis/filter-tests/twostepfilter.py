...
def predict_error_covariance(gyro_prev, z_prev, T, P, sigma_gyro):
    ...
    return (np.identity(3) - T * skew(gyro_prev)).dot(P).dot(
        (np.identity(3) - T * skew(gyro_prev)).T
    ) + Q
...
def interpolate_array(current_t, t_init, t_end, vector_init, vector_end):
    ...
    """
    return [
        interpolate(current_t, t_init, t_end, values[0], values[1])
        for values in zip(vector_init, vector_end)
    ]
